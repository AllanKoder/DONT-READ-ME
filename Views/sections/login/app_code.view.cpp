...
namespace Views
{
    AppCode::AppCode(std::shared_ptr<cgicc::Cgicc> cgi, std::string challengeCode) : View(cgi)
    {
        std::string bodyOutput = "";

        bodyOutput += "<form action='/cgi-bin/appCodePost.cgi' method='POST'>\n";
        bodyOutput += "    <div class='mb-4'>\n";
        bodyOutput += "        <label for='code' class='block text-sm font-medium text-gray-700 mb-2'>Challenge: (" + challengeCode + ")</label>\n";
