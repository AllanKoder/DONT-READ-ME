...
namespace Views
{
    AppCode::AppCode(std::shared_ptr<cgicc::Cgicc> cgi) : View(cgi)
    {
        ...
        bodyOutput += "        <label for='code' class='block text-sm font-medium text-gray-700 mb-2'>2 character code (App):</label>\n";
        ...
        bodyOutput += "            required \n";
        bodyOutput += "            pattern='[a-zA-Z0-9]{2}' \n";
        bodyOutput += "            maxlength='2' \n";
        bodyOutput += "            title='Please enter a 2 character code'>\n";
        bodyOutput += "    </div>\n";
